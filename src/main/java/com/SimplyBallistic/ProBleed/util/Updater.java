...
/**
 * This is a fork of the popular Auto Updater by Gravity.
 * <br>
 * <br>
 * It can be found here: https://github.com/gravitylow/Updater/blob/master/src/main/java/net/gravitydevelopment/updater/Updater.java
 * <br>
 * <br>
 * The download function currently does not work because of spigot's download mechanism. A fix will be made soon.
 * Make sure to cite the respective authors when using this class.
 * Enjoy!
 *
 * While it is not strictly enforced on Spigot, give your users the ability to disable the auto update feature. It is respective to do so in order to allow your
 * users to choose a version they prefer to use.
 *
 * @author Gravity
...
public class Updater {
    ...
    // User-agent when querying Spigot
    private static final String USER_AGENT = "Spigot AutoUpdater";
    // If the version number contains one of these, don't update.
    private static final String[] NO_UPDATE_TAG = { "-DEV", "-PRE", "-SNAPSHOT" };
    // Used for downloading files
    private static final int BYTE_SIZE = 1024;
    // Config key for api key. Only here for generation of bukkit config
    private static final String API_KEY_CONFIG_KEY = "api-key";
    // Config key for disabling Updater. Only here for generation of bukkit config
    private static final String DISABLE_CONFIG_KEY = "disable";
    // Default api key value in config. Won't make a difference in the Spigot version of this Updater.
    ...
    // link to download as Spigot does not work
    private static final String DOWNLOAD_LINK="https://github.com/SimplyBallistic/ProBleed/releases/download/%version%/ProBleed.jar";
    ...
    // Project's Spigot Resource ID
    private int id = -1;

    /* Collected from Spiget REST API */
    ...
    public Updater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        ...
        try {
            this.versionUrl = new URL("https://api.spiget.org/v2/resources/"+ this.id +"/versions?sort=-name");
            this.updateUrl=new URL("https://api.spiget.org/v2/resources/"+ this.id +"/updates");
        ...
        if (this.result != UpdateResult.FAIL_BADID) {
            this.thread = new Thread(new UpdateRunnable(),plugin.getName()+" update thread");
            this.thread.start();
        } else {
            runUpdater();
        }
    ...
    /**
     * Get the latest version's update summary
     *
    ...
    private void waitForThread() {
        if ((this.thread != null) && this.thread.isAlive()) {
            try {
                if(!Thread.currentThread().getName().equals(plugin.getName()+" update thread"))
                this.thread.join();
                else throw new IllegalStateException("You can't wait for the update thread to finish if you are on it!");
    ...
    private void downloadFile() {
        ...
        try {
            ...
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionValue);
            }
            long downloaded = 0;
            ...
            if(announce)
            this.plugin.getLogger().log(Level.WARNING, "The auto-updater tried to download a new update, but was unsuccessful: "+ex.getMessage());
    ...
    private boolean versionCheck() {
        final String title = this.versionValue;
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            final String localVersion = this.plugin.getDescription().getVersion();

                // Get the newest file's version number
                final String remoteVersion=this.versionValue;
     ...
     * <p>
     * With default behavior, Updater will NOT verify that a remote version available on Spiget
     * which is not this version is indeed an "update".
     * If a version is present on Spiget that is not the version that is currently running,
    ...
    private boolean read() {
        try {
            final URLConnection conn = this.versionUrl.openConnection();
            final URLConnection updateCon=this.updateUrl.openConnection();
            final URLConnection linkCon=new URL("https://api.spiget.org/v2/resources/"+this.id).openConnection();

            conn.setConnectTimeout(5000);
            updateCon.setConnectTimeout(5000);
            linkCon.setConnectTimeout(5000);

            /*if (this.apiKey != null) {
                conn.addRequestProperty("X-API-Key", this.apiKey);
            }*/
            conn.addRequestProperty("User-Agent", Updater.USER_AGENT);
            updateCon.addRequestProperty("User-Agent", Updater.USER_AGENT);
            linkCon.addRequestProperty("User-Agent", Updater.USER_AGENT);

            conn.setDoOutput(true);
            updateCon.setDoOutput(true);
            linkCon.setDoOutput(true);

            final BufferedReader reader = new BufferedReader(new InputStreamReader(conn.getInputStream()));
            final BufferedReader updateReader = new BufferedReader(new InputStreamReader(updateCon.getInputStream()));
            final BufferedReader linkReader = new BufferedReader(new InputStreamReader(linkCon.getInputStream()));
            final String response = IOUtils.toString(reader);
            final String updateResponse = IOUtils.toString(updateReader);
            final String linkResponse= IOUtils.toString(linkReader);

            final JSONArray array = (JSONArray) JSONValue.parse(response);
            final JSONArray updateArray = (JSONArray) JSONValue.parse(updateResponse);
            final JSONObject linkArray= (JSONObject) JSONValue.parse(linkResponse);

            if (linkArray.isEmpty()||linkArray.get("error")!=null) {
                this.plugin.getLogger().warning("The updater could not find any files for the project id " + this.id);
                this.result = UpdateResult.FAIL_BADID;
                return false;
            }

            JSONObject latestUpdate = (JSONObject) array.get(0);
            JSONObject jsonFile= (JSONObject) linkArray.get("file");
            JSONObject latestUpdateInfo=(JSONObject)updateArray.get(updateArray.size()-1);
            this.versionValue = (String) latestUpdate.get("name");
            this.versionName=(String) latestUpdateInfo.get("title");
            this.versionLink = "https://www.spigotmc.org/"+
                    jsonFile.get("url");


            this.versionGameVersion = linkArray.get("testedVersions").toString().replace("[","").replace("]","").replaceAll("\"","").split(",");
            return true;
        } catch (final IOException e) {
            if (e.getMessage().contains("HTTP response code: 403")) {
                this.result = UpdateResult.FAIL_APIKEY;
            } else {
                this.plugin.getLogger().severe("The updater could not contact spiget for updating.");
                this.plugin.getLogger().severe("If you have not recently modified your configuration and this is the first time you are seeing this message, the site may be experiencing temporary downtime.");
                this.result = UpdateResult.FAIL_DBO;
            }
            this.plugin.getLogger().warning("Failed checking for updates! Error: "+e.getMessage());
            return false;
        }
    }
    ...
    private void runUpdater() {
        if (this.versionUrl != null && (this.read() && this.versionCheck())) {
